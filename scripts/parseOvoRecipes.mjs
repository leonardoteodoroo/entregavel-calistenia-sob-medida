...
async function parseFile(filePath) {
  try {
    ...
    for (const block of blocks) {
      ...
      while (
        i < lines.length &&
        !lines[i].toLowerCase().includes("ingredientes")
      ) {
      ...
      if (lines[i] && lines[i].toLowerCase().includes("ingredientes")) {
        ...
        while (
          i < lines.length &&
          !lines[i].toLowerCase().includes("modo de preparo")
        ) {
          if (lines[i].trim().length > 2 && !lines[i].match(/^[@a-z\.\s]$/i)) {
            // Filtra sujeira como '@', 's', 'v.'
    ...
    console.log(
      `Lembre-se de refinar com LLM para os campos de 'premise', 'objection', 'masterTip' e 'visual'!`
    );
  } catch (error) {
