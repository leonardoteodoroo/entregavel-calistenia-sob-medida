...
export default function TipCard({ tip }: TipCardProps) {
  ...
  return (
    <div
      ...
      className={[
        styles.wrapper,
        isInView ? styles.cardVisible : styles.cardHidden,
      ].join(" ")}
    >
      <Card
      ...
      >
        <p className={styles.counter}>
          🌿 Dica #{tip.number.toString().padStart(2, "0")}
        </p>

        <blockquote className={styles.quote}>{tip.text}</blockquote>
        ...
        <Link to={`/biblioteca/${tip.alliedOil}`} className={styles.alliedOil}>
          ...
          <div>
            <p className={styles.alliedTitle}>
              Óleo Aliado: {tip.alliedOilName}
            </p>
