...
const alertIcons: OrganicIconName[] = [
  "shield",
  "capsule",
  "dropper",
  "sun",
...
function getQuoteAfterSection(sectionId: string) {
  return (
    data.pullQuotes.find(
      entry => entry.afterSection === sectionId && entry.id !== "quote-5"
    ) ?? null
  );
}
...
function DropCapParagraph({
  ...
  return (
    <p
      className={[styles.body, styles.dropCap, className]
        .filter(Boolean)
        .join(" ")}
    >
      {children}
...
function AboutSection({ section }: { section: SafetySection }) {
  ...
  return (
    <section
      ...
      className={[styles.section, styles.asymmetric].join(" ")}
    >
      ...
      <div className={styles.copyStack}>
        ...
        {remainingParagraphs.map((paragraph, index) => (
          <EditorialParagraph key={`${section.id}-${index + 1}`}>
            {paragraph}
          </EditorialParagraph>
        ))}
      </div>
    ...
    >
      <div className={styles.inlineHeader}>
        ...
        <p className={styles.eyebrow}>
          {sectionMeta.legal.eyebrow.replace("⚠️ ", "")}
        </p>
      </div>
      ...
      <div className={styles.copyStack}>
        {section.body.map((paragraph, index) =>
          index === 0 ? (
            <DropCapParagraph key={`${section.id}-${index}`}>
              {paragraph}
            </DropCapParagraph>
          ) : (
            <EditorialParagraph key={`${section.id}-${index}`}>
              {paragraph}
            </EditorialParagraph>
          )
        )}
...
export default function GuiasPage() {
  ...
  const quoteBeforeClosing = data.pullQuotes.find(
    entry => entry.id === "quote-5"
  );
    ...
    <div className={styles.page}>
      <header className={[styles.section, styles.header].join(" ")}>
        ...
        <p className={[styles.body, styles.subtitle].join(" ")}>
          Conhecimento é a melhor proteção para transformar ritual em cuidado
          real.
        </p>
      </header>
      ...
      {quoteBeforeClosing ? (
        <SectionQuote quote={quoteBeforeClosing} bridge />
      ) : null}
