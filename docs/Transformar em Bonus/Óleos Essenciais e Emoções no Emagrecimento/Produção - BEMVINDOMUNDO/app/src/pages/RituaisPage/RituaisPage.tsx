...
const topicalGuide = safety.sections.find(
  section => section.id === "guia-de-uso-topico"
);
...
export default function RituaisPage() {
  ...
  const expandedRecipe =
    tabRecipes.find(recipe => recipe.id === recipeParam) ??
    tabRecipes[0] ??
    null;
  ...
  const handleTabKeyDown = (
    ...
    switch (event.key) {
      ...
      case "ArrowLeft":
        nextIndex =
          (index - 1 + recipeTabOptions.length) % recipeTabOptions.length;
        break;
      case "Home":
        nextIndex = 0;
        break;
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className="body-md text-variant mt-4">
          29 protocolos — do café da manhã ao sono
        </p>
      </header>
        ...
        {tabRecipes.map(recipe => {
          ...
          return (
            <div key={recipe.id} className={styles.recipeStack}>
              ...
              {isExpanded ? (
                <RecipeDetail id={detailId} recipe={recipe} />
              ) : null}
            </div>
          );
        })}
