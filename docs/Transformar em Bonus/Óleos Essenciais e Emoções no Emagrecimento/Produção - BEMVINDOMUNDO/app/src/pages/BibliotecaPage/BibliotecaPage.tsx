...
function isLibraryFilter(value: string | null): value is LibraryFilter {
  return (
    value === "todos" || oilFilterOptions.some(option => option.value === value)
  );
}
...
export default function BibliotecaPage() {
  ...
  const selectedFilter: LibraryFilter = isLibraryFilter(rawFilter)
    ? rawFilter
    : "todos";
  ...
  const visibleOils = oils.filter(oil => {
    const matchesFilter =
      selectedFilter === "todos" || oil.filter === selectedFilter;
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className="body-md text-variant mt-4">
          17 óleos essenciais catalogados
        </p>
      </header>
      ...
      <section className={styles.controls}>
        ...
        <input
          ...
          onChange={event =>
            updateSearchParams(selectedFilter, event.target.value)
          }
        />
      ...
      <section aria-label="Catálogo de Óleos" className={styles.catalog}>
        ...
        {selectedOil ? (
          <OilDetail oil={selectedOil} catalogSearch={routeSearch} />
        ) : null}

        <div className={styles.resultsRow}>
          <p className="body-md text-variant">
            {visibleOils.length}{" "}
            {visibleOils.length === 1 ? "óleo encontrado" : "óleos encontrados"}
          </p>
        </div>
