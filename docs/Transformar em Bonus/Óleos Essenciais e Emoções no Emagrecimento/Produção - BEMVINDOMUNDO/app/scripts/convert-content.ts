...
interface SafetySection {
  id: string;
  title: string;
  variant: "about" | "legal" | "alerts" | "ingestion" | "topical" | "closing";
  body: string[];
  bullets?: string[];
  sources?: string[];
  sourceFiles: string[];
  ...
  return {
    aromatic:
      ...
      normalized.includes("aromat")
        ...
        : "",
    topical:
      normalized.includes("massag") ||
      normalized.includes("topic") ||
      normalized.includes("pele")
        ? summary
        : "",
    ingestion:
      normalized.includes("inger") ||
...
function parseOils(): Oil[] {
  ...
  const tagsMap = parseTags(
    "00-complementar/tags-beneficio/tags-beneficio.txt"
  );
...
function parseRecipeMetadata(): Map<string, RecipeMetadata> {
  ...
  const lines = getNonEmptyLines(
    readText("00-complementar/metadados-receitas.txt")
  );
  let currentKey = "";
  ...
  for (const line of lines) {
    ...
    if (entryMatch) {
      ...
      currentKey = entryMatch[2] ? `${stem}-${slugify(entryMatch[2])}` : stem;
...
function parseRecipeFile(
  ...
  const relativePath = absolutePath.replace(`${contentRoot}/`, "");
  const lines = readTextAbsolute(absolutePath)
    .split("\n")
    .map(line => line.trimEnd());
  const nonEmptyLines = lines.map(line => line.trim());
  const ingredientsIndex = nonEmptyLines.findIndex(
    line => line === "INGREDIENTES"
  );
  const modeIndex = nonEmptyLines.findIndex(line =>
    line.includes("MODO DE PREPARO")
  );

  const name = normalizeWhitespace(
    nonEmptyLines.slice(0, ingredientsIndex).join(" ")
  );
...
function parseDiffuserRecipes(
  ...
  return combos.map((ingredients, index) => {
    ...
    const descriptiveName =
      metadata.sopTitle.split("—")[1]?.trim() ??
      `Combinação para Difusor ${index + 1}`;
...
function parseRecipes(): Recipe[] {
  ...
  const recipes = recipeFiles.flatMap(({ dir, tab }) =>
    readTxtFiles(dir)
      .filter(
        absolutePath =>
          !absolutePath.endsWith("orientacoes-massagem-e-diluicao.txt")
      )
...
function parseTipMetadata(): Map<string, TipMetadata> {
  ...
  const lines = getNonEmptyLines(
    readText("00-complementar/metadados-dicas-mindset.txt")
  );
  let currentKey = "";
...
function parsePullQuotes(): PullQuote[] {
  const lines = readText(
    "00-complementar/pull-quotes-editoriais/pull-quotes.txt"
  )
    .split("\n")
    .map(line => line.trim());
...
function parseSafety(): SafetyData {
  ...
  const alertsBody = getNonEmptyLines(
    readText("03-apresentacao/observacoes-importantes.txt")
  );
  const alerts = alertsBody
    .filter(line => line.startsWith("- "))
    .map(line => line.slice(2));
  const ingestion = splitParagraphs(
    readText("09-ingestao-e-aromaterapia/orientacoes-seguranca.txt")
  );
  const topical = stripLeadingHeading(
    splitParagraphs(
      readText("08-uso-topico/orientacoes-massagem-e-diluicao.txt")
    )
  );
    ...
    sections: [
      {
        ...
        sourceFiles: [
          "02-introducao/introducao.txt",
          "03-apresentacao/apresentacao.txt",
        ],
      },
      {
        id: "aviso-legal",
      ...
      },
      {
        ...
        variant: "topical",
        body: topical[0]?.startsWith("Uso Tópico") ? topical.slice(1) : topical,
