...
import { Dumbbell, CalendarRange, BookOpen, TrendingUp, Sparkles, CheckCircle2, Utensils, Moon, Calculator, Mic, Download } from 'lucide-react';
import { motion } from 'framer-motion';

export default function App() {
  const [activeIndex, setActiveIndex] = useState(2);
  const [ripples, setRipples] = useState<{ [key: number]: { x: number, y: number, size: number, id: number }[] }>({});
  ...
  const toggleDay = (day: number) => {
    setCheckedDays(prev => {
      const next = prev.includes(day) ? prev.filter(current => current !== day) : [...prev, day];
    ...
    <div className="flex flex-col min-h-screen bg-[var(--color-ivory)] font-sans">
          ...
          {activeIndex === 2 ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8 max-w-4xl mx-auto">
              ...
              <section className="space-y-6">
                ...
                <p className="text-[var(--color-taupe)] text-lg leading-relaxed">
                  Versão isolada para explorar valor percebido de biblioteca de exercícios sem depender de vídeo humano em todos os movimentos. O foco aqui é clareza de execução, segurança e leitura rápida.
                </p>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={i} className="bg-white/50 p-4 rounded-2xl border border-white/80">
                      <span className="block text-[8px] font-bold uppercase tracking-widest text-[var(--color-tauest)] opacity-70">{item.label}</span>
                      <span className="block text-xs font-bold text-[var(--color-charcoal)] mt-1">{item.value}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="pt-8 space-y-6">
                <h2 className="text-2xl font-heading text-[var(--color-charcoal)]">Início, Meio e Fim</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-white/80 group transform transition-all hover:-translate-y-1">
                      <img 
              ...
              <section className="pt-8 space-y-4">
                <h2 className="text-2xl font-heading text-[var(--color-charcoal)]">Execução Correta</h2>
                <div className="bg-white/60 p-6 rounded-3xl border border-white/80 space-y-3 text-[var(--color-charcoal-light)]">
                  {[
                    ...
                    <div key={i} className="flex gap-4 items-start">
                      <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[var(--color-rose-muted)] text-[var(--color-rose)] flex items-center justify-center text-xs font-bold">{i+1}</span>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-4xl mx-auto">
               <section id="checklist" className="mb-6 mt-2">
                ...
                <div>
                  <p
                    className="font-sans mb-3 text-[0.65rem] font-medium tracking-[0.15em] uppercase text-[var(--color-taupe)]"
                  >
                ...
                <div className="mt-12 bg-white/40 p-6 rounded-3xl border border-white/80">
                  ...
                  <p className="font-heading italic text-[var(--color-taupe)] text-sm mb-8">Sua regularidade dividida por fases</p>
                  
                  <div className="flex h-36 gap-4 px-2">
                    {[1, 2, 3, 4].map(week => {
                      const daysInWeek = Array.from({ length: 7 }, (_, i) => (week - 1) * 7 + i + 1);
                      const completedCount = daysInWeek.filter(d => checkedDays.includes(d)).length;
                      const percentage = (completedCount / 7) * 100;
                      
                      return (
                        <div key={week} className="flex-1 flex flex-col items-center h-full group">
                          ...
                          >
                            ...
                            {percentage > 0 && (
                              <div className="absolute inset-x-0 bottom-1 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="text-[10px] font-bold text-white drop-shadow-md">{completedCount}/7</span>
                              </div>
                            )}
                          </div>
                          <span className="text-[10px] uppercase tracking-widest font-bold text-[var(--color-taupe)] mt-3">S0{week}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="mt-10">
                  ...
                  <p className="font-heading italic text-[var(--color-taupe)] text-sm mb-4">Clareza de evolução em 28 dias</p>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: "Dias concluídos", value: `${checkedDays.length}/28` },
                      ...
                      { label: "Semana atual", value: Math.ceil((Math.max(...checkedDays, 0) || 1) / 7) },
                      { label: "Próximo dia", value: Math.max(...checkedDays, 0) + 1 > 28 ? 28 : Math.max(...checkedDays, 0) + 1 },
                    ].map((stat, i) => (
                      <div key={i} className="bg-white p-5 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.04)] border border-[var(--color-taupe-light)] opacity-90 transition-all hover:shadow-md">
                        <p className="text-[10px] font-sans font-medium uppercase tracking-wider text-[var(--color-taupe)] mb-1.5">{stat.label}</p>
                        <p className="text-2xl font-heading font-bold text-[var(--color-charcoal)] leading-none">{stat.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-8">
                  {[1, 2, 3, 4].map(week => (
                    ...
                    >
                      <span className="text-xs font-medium text-[var(--color-charcoal)] group-hover:text-[var(--color-rose)] transition-colors">Semana {week}</span>
                      <svg 
                        width="12" 
                        height="12" 
                        viewBox="0 0 24 24" 
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8">
              <section>
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-3xl font-heading text-[var(--color-charcoal)] mb-1">Extras</h2>
                    <p className="text-[var(--color-taupe)] text-sm">Conteúdos premium para potencializar sua evolução na calistenia.</p>
                  </div>
                  <Sparkles className="text-[var(--color-rose)] opacity-50" size={20} />
                </div>
                ...
                <div className="mt-8 relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-[#536976] to-[#292E49] p-8 text-white shadow-xl">
                  <div className="relative z-10">
                    <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-bold tracking-widest uppercase mb-4">Novo</span>
                    <h3 className="text-2xl font-heading font-bold mb-3 leading-tight max-w-[200px]">Guia de Sobrevivência: Doces</h3>
                    <p className="text-white/70 text-sm mb-6 max-w-[220px]">Aprenda a lidar com a vontade de açúcar sem estragar sua dieta.</p>
                ...
                <div className="grid grid-cols-2 gap-4 mt-6">
                   <div className="bg-white p-6 rounded-[2rem] border border-[var(--color-taupe-light)] shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <h4 className="font-heading font-bold text-[var(--color-charcoal)] leading-tight mb-2">Cardápio 1500 kcal</h4>
                      <p className="text-[10px] text-[var(--color-taupe)] uppercase tracking-wider">Plano alimentar completo</p>
                   </div>
                   <div className="bg-white p-6 rounded-[2rem] border border-[var(--color-taupe-light)] shadow-sm hover:shadow-md transition-shadow">
                      <div className="w-10 h-10 rounded-2xl bg-pink-50 flex items-center justify-center text-pink-600 mb-4">
                        <Moon size={20} />
                      </div>
                      <h4 className="font-heading font-bold text-[var(--color-charcoal)] leading-tight mb-2">Alongamento Sono</h4>
                      <p className="text-[10px] text-[var(--color-taupe)] uppercase tracking-wider">Rotina relaxante noturna</p>
                   </div>
                   <div className="bg-white p-6 rounded-[2rem] border border-[var(--color-taupe-light)] shadow-sm hover:shadow-md transition-shadow">
                      <div className="w-10 h-10 rounded-2xl bg-orange-50 flex items-center justify-center text-orange-600 mb-4">
                        <Calculator size={20} />
                      </div>
                      <h4 className="font-heading font-bold text-[var(--color-charcoal)] leading-tight mb-2">Calculadora TMB</h4>
                      <p className="text-[10px] text-[var(--color-taupe)] uppercase tracking-wider">Calcule seu gasto diário</p>
                   </div>
                </div>

                {/* Final List */}
                <div className="mt-10">
                   <h2 className="text-xl font-heading text-[var(--color-charcoal)] mb-6">Mais Conteúdos</h2>
                   <div className="space-y-4">
                      <div className="bg-white flex items-center justify-between p-4 rounded-2xl border border-[var(--color-taupe-light)] shadow-sm">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h5 className="text-sm font-bold text-[var(--color-charcoal)]">Podcast: Mente Blindada</h5>
                            <p className="text-[10px] text-[var(--color-taupe)]">Áudio • 12 min</p>
                          </div>
                        </div>
                        <button className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 transition-colors">
                          <TrendingUp size={14} className="rotate-90" /> {/* Symbolizing play or icon from image */}
                        </button>
                      </div>
                      ...
                      <div className="bg-white flex items-center justify-between p-4 rounded-2xl border border-[var(--color-taupe-light)] shadow-sm">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h5 className="text-sm font-bold text-[var(--color-charcoal)]">Checklist: Treino de Viagem</h5>
                            <p className="text-[10px] text-[var(--color-taupe)]">PDF • 1.2 MB</p>
                          </div>
                        </div>
                        <button className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 transition-colors">
                          <Download size={14} />
            ...
            <div className="flex items-center justify-center min-h-[50vh] text-[var(--color-taupe)] animate-pulse">
              <span className="text-xl font-heading italic">Conteúdo de {menus[activeIndex].name} em desenvolvimento...</span>
            </div>
          )}
        </div>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 h-[42px] bg-white flex justify-center shadow-[0_-4px_12px_rgba(0,0,0,0.03)] overflow-hidden z-50">
        <ul className="flex w-[350px] relative">
          ...
          >
             <svg width="150" height="42" viewBox="0 0 150 42" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_8px_20px_rgba(139,74,82,0.35)] overflow-visible">
              <defs>
                ...
                <div className="relative flex flex-col justify-center items-center w-full h-full">
                  <span 
                    className={`relative block transition-all duration-500 ${
                      isActive ? '-translate-y-2.5 scale-110' : 'translate-y-0 scale-100'
                    }`}
                  ...
                  <span 
                    className={`absolute text-[8px] transition-all duration-500 whitespace-nowrap ${
                      isActive ? 'opacity-100 translate-y-3 text-white font-medium' : 'opacity-0 translate-y-8'
                    }`}
                    style={{ transitionTimingFunction: 'cubic-bezier(0.68, -0.55, 0.265, 1.55)' }}
                  >
                     {menu.name}
