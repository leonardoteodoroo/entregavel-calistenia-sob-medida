...
export default function SharedSidebar({
  ...
  useEffect(() => {
    setExpandedWeeks(prev => expandWeekFromLocation(prev, currentLocation));
  }, [currentLocation]);
  ...
  const navigateTo = (to: string) => {
    const isSameRoute = currentLocation === to;
    onNavigate(to);
    setSidebarOpen(false);

    if (to === "/bonus" && isSameRoute) {
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3">
          {navSections.map((section, sectionIndex) => {
            ...
            const hasActiveChild = Boolean(
              section.children?.some(child => currentLocation === child.to)
            );
            const isActive =
              isRouteActive(currentLocation, sectionTo) || hasActiveChild;
